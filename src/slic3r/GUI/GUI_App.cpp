...
/*#ifdef WIN32
    ...
    try {
        ...
        flush_logs();
        wxString errmsg = wxString::Format(_L("PollenAMOrcaSlicer will terminate because of running out of memory. "
        ...
        //throw;
     	...
     	BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        	flush_logs();
        wxString errmsg = _L("PollenAMOrcaSlicer will terminate because of a localization error. "
        ...
        BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        flush_logs();
        wxLogError(format_wxstr(_L("PollenAMOrcaSlicer got an unhandled exception: %1%"), ex.what()));
        throw;
    }
//#endif
}
    ...
    if (is_editor() && m_last_config_version && m_last_config_version->valid()
        ...
        CallAfter([] {
            ...
            MessageDialog dlg(nullptr,
                _L("Since version 2.4.0, PollenAMOrcaSlicer syncs user profiles through Orca Cloud instead of Bambu Cloud.\n\n"
                   "To migrate your existing profiles, log in to Orca Cloud and they will be transferred automatically. "
                   "To learn more about how PollenAMOrcaSlicer stores and syncs your profiles, or to migrate your presets manually, check out our wiki.\n\n"
...
void GUI_App::init_webview_runtime()
{
    ...
    int nRet = wxMessageBox(_L("PollenAMOrcaSlicer requires the Microsoft WebView2 Runtime to operate certain features.\nClick Yes to install it now."),
    ...
    if (installed && WebView::CheckWebViewRuntime()) {
        ...
        wxMessageBox(_L("The Microsoft WebView2 Runtime could not be installed.\n"
                        ...
                        "Please install it manually from https://developer.microsoft.com/microsoft-edge/webview2/ and restart PollenAMOrcaSlicer."),
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
    ...
    if (is_editor()) {
            ...
            if (this->plater_ != nullptr) {
               ...
               /* wxString tips = wxString::Format(_L("Click to download new version in default browser: %s"), version_info.version_str);
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("New version of PollenAMOrcaSlicer"),
                    false,
        ...
        Bind(EVT_ENTER_FORCE_UPGRADE, [this](const wxCommandEvent& evt) {
                ...
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("PollenAMOrcaSlicer needs an update"),
                    false,
                    wxCENTER | wxICON_INFORMATION);
...
//#endif
    ...
    if (m_config_corrupted) {
        ...
        show_error(nullptr,
                   _u8L(
                       "The PollenAMOrcaSlicer configuration file may be corrupted and cannot be parsed.\nPollenAMOrcaSlicer has attempted to recreate the "
...
void GUI_App::on_http_error(wxCommandEvent &evt)
{
    ...
    if (code == HttpErrorVersionLimited) {
        MessageDialog msg_dlg(nullptr, _L("The version of PollenAMOrcaSlicer is too low and needs to be updated to the latest version before it can be used normally."), "", wxAPPLY | wxOK);
        if (msg_dlg.ShowModal() == wxOK) {
        }
    }
...
bool GUI_App::process_network_msg(std::string dev_id, std::string msg)
{
    if (dev_id.empty()) {
        if (msg == "wait_info") {
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("Please try updating PollenAMOrcaSlicer and then try again."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("The certificate has expired. Please check the time settings or update PollenAMOrcaSlicer and try again."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("Internal error. Please try upgrading the firmware and PollenAMOrcaSlicer version. If the issue persists, contact support."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            MessageDialog
                msg_dlg(nullptr,
                        _L("To use PollenAMOrcaSlicer with Bambu Lab printers, you need to enable LAN mode and Developer mode on your printer.\n\n"
                           "Please go to your printer's settings and:\n"
                           "1. Turn on LAN mode\n"
                           "2. Enable Developer mode\n\n"
                           "Developer mode allows the printer to work exclusively through local network access, "
                           "enabling full functionality with PollenAMOrcaSlicer."),
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (!wxLocale::IsAvailable(locale_language_info->Language)) {
	    ...
	    wxString message = "Switching PollenAMOrcaSlicer to language " + requested_language_code + " failed.";
        ...
        wxMessageBox(message, "PollenAMOrcaSlicer - Switching language failed", wxOK | wxICON_ERROR);
        if (initial)
...
void GUI_App::associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id = L" Orca.Slicer.1";
    std::wstring prog_desc = L"" SLIC3R_APP_FULL_NAME;
