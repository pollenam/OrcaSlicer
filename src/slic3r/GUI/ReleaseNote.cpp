    ...
    : DPIDialog(static_cast<wxWindow*>(wxGetApp().mainframe), wxID_ANY, _L("Network plug-in update"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    operation_tips = new ::Label(this, Label::Body_12, _L("Click OK to update the Network plug-in the next time PollenAMOrcaSlicer launches."), LB_AUTO_WRAP);
...
UpdateVersionDialog::UpdateVersionDialog(wxWindow *parent)
    : DPIDialog(parent, wxID_ANY, _L("New version of PollenAMOrcaSlicer"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX | wxRESIZE_BORDER)
{
    SetBackgroundColour(*wxWHITE);
...
void UpdateVersionDialog::update_version_info(wxString release_note, wxString version)
{
    ...
    url_line          = "";
    // Orca: not used in PollenAMOrcaSlicer
    ...
    if (is_running_in_msix())
        m_text_up_info->SetLabel(wxString::Format(_L("New version available: %s. Please update PollenAMOrcaSlicer from the Microsoft Store."), version));
    else
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    comfirm_before_enter_text = _L("1. Please confirm PollenAMOrcaSlicer and your printer are in the same LAN.");
    ...
    m_test_right_msg->SetForegroundColour(wxColour(255, 179, 0));
