...
void Sidebar::sync_ams_list(bool is_from_big_sync_btn)
{
    ...
    if (!unknowns.empty()) {
        MessageDialog dlg(this,
            _L("There are some unknown or incompatible filaments mapped to generic preset.\nPlease update PollenAMOrcaSlicer or restart PollenAMOrcaSlicer to check if there is an update to system presets.") + detail,
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
    if (!sync_color_only) {
        ...
        try {
            if (type_3mf) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    const wxString msg_unsupported_geometry    = _L("The 3MF is not supported by PollenAMOrcaSlicer, loading geometry data only.");
                    const wxString msg_old_orca_geometry       = _L("The 3MF file was generated by an old PollenAMOrcaSlicer version, loading geometry data only.");
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if (load_config && (file_version < app_version) && file_version == Semver("2.3.1-alpha")) {
                            if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                                ...
                                if (!is_safe_to_rotate) {
                                    wxString msg_text = _(
                                        L("This project was created with an PollenAMOrcaSlicer 2.3.1-alpha and uses "
                                    ...
                                    msg_text += "\n\n" +
                                                _(L("Would you like PollenAMOrcaSlicer to automatically fix this by clearing the rotation template settings?"));
                        ...
                        if (file_version <= orca_tag_start_version) {
                            ...
                            if (load_config && config_loaded.empty()) {
                                ...
                                wxString text = _L("The 3MF was created by BambuStudio. Some settings may differ from PollenAMOrcaSlicer.");
                                log_and_show_3mf_info(text, bambu_project_title);
                            }
                        }
                    }
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "PollenAMOrcaSlicer window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to PollenAMOrcaSlicer failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
    ...
    if (type == PrinterWarningType::NOT_CONNECTED) {
        if (device_page) {
            ...
            content = wxString::Format(
                _L("PollenAMOrcaSlicer can't connect to %s. Please check if the printer is powered on and connected to the network."), printer_name);
        }
