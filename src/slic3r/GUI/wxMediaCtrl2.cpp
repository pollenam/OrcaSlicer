...
wxMediaCtrl2::wxMediaCtrl2(wxWindow *parent)
{
    ...
    if (m_native_wayland && is_gstreamer_feature_available("gtksink"))
        ...
        m_gtk_sink_error = _L("Native Wayland liveview requires the GStreamer GTK video sink. Please install the gtksink plugin for GStreamer, then restart PollenAMOrcaSlicer.");
...
void wxMediaCtrl2::Load(wxURI url)
{
    ...
    }
    {
        ...
        if (path.empty() || !wxFile::Exists(path) || clsid != CLSID_BAMBU_SOURCE) {
            if (boost::filesystem::exists(dll_path)) {
                ...
                CallAfter([] {
                    wxMessageBox(_L("Missing BambuSource component registered for media playing! Please re-install PollenAMOrcaSlicer or seek community help."), _L("Error"), wxOK);
                });
            }
    ...
    if (!hasplugins) {
        CallAfter([] {
            wxMessageBox(_L("Your system is missing H.264 codecs for GStreamer, which are required to play video. (Try installing the gstreamer1.0-plugins-bad or gstreamer1.0-libav packages, then restart PollenAMOrcaSlicer?)"), _L("Error"), wxOK);
        });
        m_error = 101;
        wxMediaEvent event(wxEVT_MEDIA_STATECHANGED);
        event.SetId(GetId());
